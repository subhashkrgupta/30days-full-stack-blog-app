...
const Dashboard = () => {
  ...
  return (
    ...
    <div className="min-h-screen bg-[#f8fafc] dark:bg-gray-950 pb-12 transition-colors duration-300">
      
      {/* Top Navigation Bar */}
      <nav className="bg-white dark:bg-gray-900 border-b border-slate-200 dark:border-gray-800 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16 items-center">
            ...
            <button
              onClick={() => navigate("/")}
              className="flex items-center gap-2 text-slate-600 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors font-medium text-sm"
            >
              <Home size={18} />
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8">
        ...
        <header className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            ...
            <p className="text-slate-500 dark:text-gray-400 mt-2 max-w-md">
              Welcome back to your creator studio. Here’s what’s happening with your content.
            </p>
          </div>
          <button
            onClick={() => navigate("/dashboard/create")}
            className="inline-flex items-center justify-center gap-2 bg-indigo-600 dark:bg-indigo-500 hover:bg-indigo-700 dark:hover:bg-indigo-600 text-white px-5 py-2.5 rounded-xl font-semibold shadow-sm shadow-indigo-200 dark:shadow-none transition-all active:scale-95"
          >
            <Plus size={20} />
            Create New Post
          </button>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
          <div className="bg-white dark:bg-gray-900 p-6 rounded-2xl border border-slate-200 dark:border-gray-800 shadow-sm">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm font-medium text-slate-500 dark:text-gray-400">Total Posts</p>
                <h3 className="text-2xl font-bold text-slate-900 dark:text-white">{userPostCount}</h3>
              </div>
            </div>
          </div>
          <div className="bg-white dark:bg-gray-900 p-6 rounded-2xl border border-slate-200 dark:border-gray-800 shadow-sm">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm font-medium text-slate-500 dark:text-gray-400">Latest Update</p>
          ...
          <div className="bg-white dark:bg-gray-900 p-6 rounded-2xl border border-slate-200 dark:border-gray-800 shadow-sm sm:col-span-2 lg:col-span-1">
             <div className="flex items-center gap-4">
              <div className="p-3 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 rounded-xl">
                <UserIcon size={24} />
              </div>
              <div>
                <p className="text-sm font-medium text-slate-500 dark:text-gray-400">Profile Status</p>
                <h3 className="text-lg font-bold text-slate-900 dark:text-white capitalize">{user?.role || "Active"}</h3>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="overflow-x-auto">
            {loading ? (
              <div className="p-20 text-center">
                ...
                <p className="mt-4 text-slate-500 dark:text-gray-400 font-medium">Fetching your masterpieces...</p>
              </div>
            ) : (
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50/50 dark:bg-gray-800/50 text-slate-500 dark:text-gray-400 uppercase text-xs font-semibold tracking-wider">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-gray-800">
                  {posts.map((post) => (
                    <tr key={post._id} className="hover:bg-slate-50/80 dark:hover:bg-gray-800/50 transition-colors">
                      <td className="px-6 py-4">
                        <span className="font-semibold text-slate-800 dark:text-gray-200 line-clamp-1">{post.title}</span>
                      </td>
                      <td className="px-6 py-4">
                        <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-slate-100 dark:bg-gray-800 text-slate-600 dark:text-gray-300 capitalize">
                    ...
                    <tr>
                      <td colSpan={5} className="px-6 py-16 text-center">
                        <div className="flex flex-col items-center">
                          <div className="w-16 h-16 bg-slate-50 dark:bg-gray-800 rounded-full flex items-center justify-center mb-4 text-slate-300 dark:text-gray-600">
                             <FileText size={32} />
                          </div>
                          <p className="text-slate-500 dark:text-gray-400 font-medium">No blogs found. Start writing today!</p>
                        </div>
                      </td>
                    </tr>
                  )}
